...
class zemesPurchaseItemNamesClass{

    function __construct() {}

    function getPurchaseItemNameById($id) {
        if(!is_numeric($id)) return false;
        $query = "SELECT * FROM ".zemesdb::$prefix."zememailsystem_purchaseitemnames WHERE id = ".$id;
        zememailsystem::$items[0] = zemesdb::get_row($query);
        return;
    }

    function getAllPurchaseItemNames() {
        ...
        //pagination
        $query = "SELECT COUNT(id) FROM ".zemesdb::$prefix."zememailsystem_purchaseitemnames";
        $query .= $inquery;
        ...
        //data
        $query = "SELECT * FROM `".zemesdb::$prefix."zememailsystem_purchaseitemnames`";
        $query .= $inquery;
    ...
    function isPurchaseItemExist($name) {
        $query = "SELECT COUNT(id) FROM ".zemesdb::$prefix."zememailsystem_purchaseitemnames WHERE itemname = '$name'";
    ...
    function storePurchaseItemName($data) {
        if(empty($data)) return false;

        if($data['id'] == ''){
            $result = $this->isPurchaseItemExist($data['itemname']);
            if ($result == true){
                return ALREADY_EXIST;
            }
            $query = "SELECT MAX(`ordering`) FROM `". zemesdb::$prefix."zememailsystem_purchaseitemnames`";
        ...
        $row = zemesincluder::getTable('purchaseitemnames');
    ...
    function canDeletePurchaseItemName($id){
        if(!is_numeric($id)) return false;
        $query = "SELECT COUNT(id) FROM `". zemesdb::$prefix."zememailsystem_purchases` WHERE purchaseitemnameid = ".$id;
        $total = zemesdb::get_var($query);
        if(is_numeric($total)){if ($total > 0) {$result = false; }else{$result = true; } }else{$result = false; }
        return $result;
    }

    function deletePurchaseItemName($id) {
        if(!is_numeric($id)) return false;
        $row = zemesincluder::getTable('purchaseitemnames');
        if ($this->canDeletePurchaseItemName($id)) {
            if (!$row->delete($id)) {
                return DELETE_ERROR;
            }
        }else{
    ...
    function getPurchaseItemNameCombo() {
        $query = "SELECT id , itemname AS text FROM `". zemesdb::$prefix."zememailsystem_purchaseitemnames` WHERE status = 1 ORDER BY ordering ASC";
