...
<div id="jjes-main-area">
	<div class="es-admin-title"><?php echo __('Purchase Items List','zem_emailsystem');?><a class="es-add-button-link" href="?page=purchaseitemnames&zemel=formpurchaseitemname"><i class="fa fa-plus-circle fa-lg fa-fw"></i>&nbsp;<?php echo __('Add New Purchase item','zem_emailsystem');?></a></div>
	<div class="jjes-content-area">
		<form name="jjes-searchform" id="jjes-searchform" method="post" action="<?php echo admin_url("admin.php?page=purchaseitemnames"); ?>">
			<?php
            ...
            <tbody>
				<?php
					...
					for ( $i = 0; $i < $total; $i++ ){
						...
						<tr valign="top">
							<td class="left-row">
							 	<a href="?page=purchaseitemnames&zemel=formpurchaseitemname&zemesid=<?php echo $row->id; ?>"> <?php echo $row->itemname; ?></a>
							</td>
							<td class="centered">
							...
							<td class="centered">
								<?php 
								if($i != 0){ ?>
					    			<a href="?page=purchaseitemnames&task=updateordering&action=zemesaction&order=up&zemesid=<?php echo $row->id; ?>"><i style="color: #2980b9" class="fa fa-sort-asc fa-fw"></i></a>
								<?php
								} 
								echo $row->ordering;
								if($i != $total - 1){ ?>
					    			<a href="?page=purchaseitemnames&task=updateordering&action=zemesaction&order=down&zemesid=<?php echo $row->id; ?>"><i style="color: #2980b9" class="fa fa-sort-desc fa-fw"></i></a>
								<?php
								} ?>
							</td>
							<td class="centered">
					    	...
					    	<td class="action">
					    		<a href="?page=purchaseitemnames&zemel=formpurchaseitemname&zemesid=<?php echo $row->id; ?>"><i style="color:black;" class="fa fa-pencil fa-lg fa-fw"></i></a>
					    		<a class="zem-confirm" href="?page=purchaseitemnames&task=remove&action=zemesaction&zemesid=<?php echo $row->id; ?>"><i style="color:black;" class="fa fa-trash-o fa-lg fa-fw"></i></a>
				    		</td>
						</tr> <?php 
					} ?>             
            </tbody>
