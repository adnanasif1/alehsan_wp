...
class zemesStatsClass{
    ...
    function storeStats($data) {
        ...
        if($data['id']==''){
            $result = $this->isStatsExist($data['name']);
            if ($result == true){
    ...
    function deleteStats($id) {
        if(!is_numeric($id)) return false;
        $row = zemesincluder::getTable('phonebook');
        if ($this->canDeleteStats($id) == true) {
            if (!$row->delete($id)) {
                return DELETE_ERROR;
            }
        }else{
    ...
    function canDeleteStats($id) {
        return true;
    }

    function isStatsExist($name) {
