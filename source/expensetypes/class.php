...
class zemesExpenseTypesClass{

    function __construct() {}

    function getExpenseTypeById($id) {
        if(!is_numeric($id)) return false;
        $query = "SELECT * FROM ".zemesdb::$prefix."zememailsystem_expensetypes WHERE id = ".$id;
    ...
    function getAllExpenseTypes() {
        ...
        //pagination
        $query = "SELECT COUNT(id) FROM ".zemesdb::$prefix."zememailsystem_expensetypes";
        $query .= $inquery;
        ...
        //data
        $query = "SELECT * FROM `".zemesdb::$prefix."zememailsystem_expensetypes`";
        $query .= $inquery;
    ...
    function isExpenseItemExist($name) {
        $query = "SELECT COUNT(id) FROM ".zemesdb::$prefix."zememailsystem_expensetypes WHERE itemname = '$name'";
    ...
    function storeExpenseType($data) {
        ...
        if($data['id'] == ''){
            ...
            $query = "SELECT MAX(`ordering`) FROM `". zemesdb::$prefix."zememailsystem_expensetypes`";
        ...
        $row = zemesincluder::getTable('expensetypes');
    ...
    function canDeleteExpenseType($id){
        if(!is_numeric($id)) return false;
        $query = "SELECT COUNT(id) FROM `". zemesdb::$prefix."zememailsystem_expenses` WHERE expensetypeid = ".$id;
        $total = zemesdb::get_var($query);
    ...
    function getExpenseTypesCombo() {
        $query = "SELECT id , itemname AS text FROM `". zemesdb::$prefix."zememailsystem_expensetypes` WHERE status = 1 ORDER BY ordering ASC";
